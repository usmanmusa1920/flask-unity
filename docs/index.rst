...
It will create a parent directory with the name `Schoolsite` also inside the `Schoolsite` directory there is a directory with thesame name of the parent directory `Schoolsite` this directory is the one that most of configurations, registering and other thing that are going to be implemented.
